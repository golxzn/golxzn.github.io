
const primitives = {
	get_layout_for(name) {
		switch (name) {
			case "3D": return [
				new attribute_layout({ count: 3, type: gl.FLOAT,          normalized: false }),
				new attribute_layout({ count: 4, type: gl.BYTE,           normalized: true }),
				new attribute_layout({ count: 2, type: gl.UNSIGNED_SHORT, normalized: true })
			];
			case "3D_COLOR": return [
				new attribute_layout({ count: 3, type: gl.FLOAT,         normalized: false }),
				new attribute_layout({ count: 4, type: gl.BYTE,          normalized: true }),
				new attribute_layout({ count: 4, type: gl.UNSIGNED_BYTE, normalized: true })
			];
			case "3D_UV": return [
				new attribute_layout({ count: 3, type: gl.FLOAT,          normalized: false }),
				new attribute_layout({ count: 4, type: gl.UNSIGNED_BYTE,  normalized: true }),
				new attribute_layout({ count: 2, type: gl.FLOAT,          normalized: false })
			];

			case "3D_SEQUENTIAL": return [
				new attribute_layout({ count: 3, type: gl.FLOAT,          normalized: false }),
				new attribute_layout({ count: 4, type: gl.BYTE,           normalized: true }),
				new attribute_layout({ count: 2, type: gl.UNSIGNED_SHORT, normalized: true })
			];
		}
		return [];
	},

	make_plane_sequential() {
		const buffer = new Uint8Array([
			//  [         x          ]  [         y          ]  [         z          ]
				0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xBF,
				0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xBF,
				0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F,
				0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F,


			//  [ normal 3 + offset  ]
				0x00, 0x7F, 0x00, 0x00,
				0x00, 0x7F, 0x00, 0x00,
				0x00, 0x7F, 0x00, 0x00,
				0x00, 0x7F, 0x00, 0x00,

			//  [         UV          ]
				0x00, 0x00, 0x00, 0x00,
				0x00, 0x00, 0xFF, 0xFF,
				0xFF, 0xFF, 0x00, 0x00,
				0xFF, 0xFF, 0xFF, 0xFF,

			// Elements Array buffer
				0x00, 0x00,
				0x00, 0x01,
				0x00, 0x02,
				0x00, 0x03
		]);

		return {
			// pipeline: ["3D", "LIGHTING"],
			draw_method: new draw_method({
				type: draw_method_type.elements,
				mode: gl.TRIANGLE_STRIP,
				target_buffer: "ebo"
			}),
			buffer_infos: [
				new buffer_info({
					name: "vbo",
					layout: primitives.get_layout_for("3D_SEQUENTIAL"),
					views: [
						new buffer_view({ offset: 0, length: 48 }),
						new buffer_view({ offset: 48, length: 16 }),
						new buffer_view({ offset: 64, length: 16 }),
					],
					accessors: [

					],
					binary: buffer, /*new Uint8Array([
					//  [         x          ]  [         y          ]  [         z          ]
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xBF,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xBF,
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F,


					//  [ normal 3 + offset  ]
						0x00, 0x7F, 0x00, 0x00,
						0x00, 0x7F, 0x00, 0x00,
						0x00, 0x7F, 0x00, 0x00,
						0x00, 0x7F, 0x00, 0x00,

					//  [         UV          ]
						0x00, 0x00, 0x00, 0x00,
						0x00, 0x00, 0xFF, 0xFF,
						0xFF, 0xFF, 0x00, 0x00,
						0xFF, 0xFF, 0xFF, 0xFF
					]) */
				}),
				new buffer_info({
					name: "ebo",
					target: gl.ELEMENT_ARRAY_BUFFER,
					views: [
						new buffer_view({ offset: 80, length: 8 })
					],
					// binary: new Uint16Array([ 0, 1, 2, 3 ])
					binary: buffer
				})
			]
		}
	},


	make_plane() {
		return {
			// pipeline: ["3D", "LIGHTING"],
			draw_method: new draw_method({
				type: draw_method_type.elements,
				mode: gl.TRIANGLE_STRIP,
				target_buffer: "ebo"
			}),
			buffer_infos: [
				new buffer_info({
					name: "vbo",
					layout: primitives.get_layout_for("3D"),
					binary: new Uint8Array([
					//  [         x          ]  [         y          ]  [         z          ]  [ normal 3 + offset  ]  [         UV          ]
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x7F, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF,
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x7F, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x7F, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF
					])
				}),
				new buffer_info({
					name: "ebo",
					target: gl.ELEMENT_ARRAY_BUFFER,
					binary: new Uint16Array([ 0, 1, 2, 3 ])
				})
			]
		}
	},

	make_custom_plane(position_scale, uv_scale) {
		// Original plane data (vertex positions, normals, UVs)
		let plane_data = new Uint8Array([
		//  [         x          ]  [         y          ]  [         z          ]  [ normal 3 + offset  ]  [                    UV                      ]
			0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F,
			0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F
		]);

		const stride = 24;
		for (var i = 0; i < 4; ++i) {
			let position = new Float32Array(plane_data.buffer, i * stride, 3);
			for (let j = 0; j < position.length; j++) {
				position[j] *= position_scale;
			}

			let uv = new Float32Array(plane_data.buffer, i * stride + 16, 2);
			for (let j = 0; j < uv.length; j++) {
				uv[j] *= uv_scale;
			}
		}

		return {
			// pipeline: ["3D", "LIGHTING"],
			draw_method: new draw_method({
				type: draw_method_type.elements,
				mode: gl.TRIANGLE_STRIP,
				target_buffer: "ebo"
			}),
			buffer_infos: [
				new buffer_info({
					name: "vbo",
					layout: primitives.get_layout_for("3D_UV"),
					binary: plane_data
				}),
				new buffer_info({
					name: "ebo",
					target: gl.ELEMENT_ARRAY_BUFFER,
					binary: new Uint16Array([ 0, 1, 2, 3 ])
				})
			]
		}
	},

	make_cube() {
		return {
			// pipeline: ["3D", "LIGHTING"],
			draw_method: new draw_method({
				type: draw_method_type.elements,
				mode: gl.TRIANGLES,
				target_buffer: "ebo"
			}),
			buffer_infos: [
				new buffer_info({
					name: "vbo",
					layout: primitives.get_layout_for("3D"),
					binary: new Uint8Array([
						,
					])
				}),
				new buffer_info({
					name: "ebo",
					target: gl.ELEMENT_ARRAY_BUFFER,
					binary: new Uint16Array([
						 0,  1,  2,  2,  1,  3, // Front face
						 4,  5,  6,  6,  5,  7, // Back face
						 8,  9, 10, 10,  9, 11, // Top face
						12, 13, 14, 14, 13, 15, // Bottom face
						16, 17, 18, 18, 17, 19, // Left face
						20, 21, 22, 22, 21, 23  // Right face
					])
				})
			]
		}
	},

	make_cube_optimized() {
		return {
			// pipeline: ["3D", "LIGHTING"],
			draw_method: new draw_method({
				type: draw_method_type.elements,
				mode: gl.TRIANGLES,
				target_buffer: "ebo"
			}),
			buffer_infos: [
				new buffer_info({
					name: "vbo",
					layout: primitives.get_layout_for("3D"),
					binary: new Uint8Array([
					//  [         x          ]  [         y          ]  [         z          ]  [ normal 3 + offset  ]  [         UV          ]
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x81, 0x00, 0x00, 0x00, 0x00, 0x00, // 0
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x81, 0x00, 0xFF, 0xFF, 0x00, 0x00, // 1
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x81, 0x00, 0x00, 0x00, 0xFF, 0xFF, // 2
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x81, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, // 3
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x7F, 0x00, 0xFF, 0xFF, 0x00, 0x00, // 4
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x00, // 5
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x7F, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, // 6
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x7F, 0x00, 0x00, 0x00, 0xFF, 0xFF, // 7
					])
				}),
				new buffer_info({
					name: "ebo",
					target: gl.ELEMENT_ARRAY_BUFFER,
					binary: new Uint16Array([
						0, 1, 2, 2, 1, 3, // Front face
						4, 5, 6, 6, 5, 7, // Back face
						2, 3, 7, 7, 3, 6, // Top face
						5, 4, 0, 0, 4, 1, // Bottom face
						1, 4, 3, 3, 4, 6, // Left face
						0, 2, 5, 5, 2, 7 // Right face
					])
				})
			]
		}
	},

	make_cube_strip() {
		return {
			// pipeline: ["3D", "LIGHTING"],
			draw_method: new draw_method({
				type: draw_method_type.elements,
				mode: gl.TRIANGLE_STRIP,
				target_buffer: "ebo"
			}),
			buffer_infos: [
				new buffer_info({
					name: "vbo",
					layout: primitives.get_layout_for("3D"),
					binary: new Uint8Array([
					//  [         x          ]  [         y          ]  [         z          ]  [ normal 3 + offset  ]  [         UV          ]
						,
					])
				}),
				new buffer_info({
					name: "ebo",
					target: gl.ELEMENT_ARRAY_BUFFER,
					binary: new Uint16Array([ 0, 1, 2, 3, 6, 1, 4, 5, 6, 7, 2, 5, 0, 1 ])
				})
			]
		}
	},

	make_cube_colored(color) {
		const to_byte = function(flt) { return parseInt(255 * flt); }

		const r = to_byte(color[0]);
		const g = to_byte(color[1]);
		const b = to_byte(color[2]);
		const a = color.length >= 4 ? to_byte(color[3]) : 0xFF;

		return {
			pipeline: ["3D", "PRIMITIVE"],
			draw_method: new draw_method({
				type: draw_method_type.elements,
				mode: gl.TRIANGLES,
				target_buffer: "ebo"
			}),
			buffer_infos: [
				new buffer_info({
					name: "vbo",
					layout: primitives.get_layout_for("3D_COLOR"),
					binary: new Uint8Array([
					//  [         x          ]  [         y          ]  [         z          ]  [ normal 3 + offset  ]  [  color ]
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x00, r, g, b, a,

						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x7F, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x7F, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x7F, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x7F, 0x00, r, g, b, a,

						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x7F, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x7F, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x7F, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x7F, 0x00, 0x00, r, g, b, a,

						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x80, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x80, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x80, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x80, 0x00, 0x00, r, g, b, a,

						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x80, 0x00, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x80, 0x00, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x80, 0x00, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x80, 0x00, 0x00, 0x00, r, g, b, a,

						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0xBF, 0x7F, 0x00, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x00, 0x00, 0x80, 0x3F, 0x7F, 0x00, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0xBF, 0x7F, 0x00, 0x00, 0x00, r, g, b, a,
						0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x7F, 0x00, 0x00, 0x00, r, g, b, a,
					])
				}),
				new buffer_info({
					name: "ebo",
					target: gl.ELEMENT_ARRAY_BUFFER,
					binary: new Uint16Array([
						 0,  1,  2,  2,  1,  3, // Front face
						 4,  5,  6,  6,  5,  7, // Back face
						 8,  9, 10, 10,  9, 11, // Top face
						12, 13, 14, 14, 13, 15, // Bottom face
						16, 17, 18, 18, 17, 19, // Left face
						20, 21, 22, 22, 21, 23  // Right face
					])
				})
			]
		}
	}

}